...
$forums = mysqli_fetch_array($result); //i7ot les éléments sélectonnées fi haja kima tab
    ...
    <div class="lg:w-2/3 flex-shirink-0">
        ...
        <ul class="card divide-y divide-gray-100 sm:m-0 -mx-5"> 
          ...
          <li> 
                ...
                <?php 
                if ($forlogin[0][0]==$forums[0]) {
                ?>
                <a href="<?php $_SERVER['PHP_SELF'] ?>?del=<?= ($forums[0]); ?>">X</a>
                <?php } ?>
                   <img src="../assets/images/avatars/avatar-2.jpg" alt="" class="w-12 h-12 rounded-full">
                    <div class="flex-1">
                        <a href="com.php" class="text-lg font-semibold line-clamp-1 mb-1"><?= strtoupper($forums[1]); ?> </a>
                        <p class="text-sm text-gray-400 mb-2"> Posted By: <span data-href="%40tag-dev.html"><?= strtoupper($forums[2]); ?> </p>
                      <p class="leading-6 mt-3"><?= ($forums[3]); ?></p>
                    </div>
